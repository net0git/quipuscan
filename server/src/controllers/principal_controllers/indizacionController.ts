...
class IndizacionController{
    ...
    public async ModificarIndizacionIndex(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
            const { indizacion, observaciones} = req.body;
            const consulta = `
                 UPDATE t_indizacion
                  SET  indizacion=$1,observaciones=$2
                 WHERE id_indizacion=$3;
                
                `;
            const valores = [indizacion,observaciones,id];
